...
run_experiment() {
	...
	#sleep 1
	#aprun -n $PROCESSES hyperPraw -n $EXPERIMENT_NAME"_prawV_default" -h $HYPERGRAPH_FILE -i 100 -m 1200 -p prawV -t $E_SIM_STEPS -x $H_SIM_STEPS_MULT -s $SEED -k $MESSAGE_SIZE -H -b $BM_FILE
	#sleep 1
	...
	sleep 1
	aprun -n $PROCESSES hyperPraw -n $EXPERIMENT_NAME"_prawE_default" -h $HYPERGRAPH_FILE -i 100 -m 1200 -p prawE -t $E_SIM_STEPS -x $H_SIM_STEPS_MULT -s $SEED -k $MESSAGE_SIZE -H -b $BM_FILE
	sleep 1
}

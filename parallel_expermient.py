...
run_experiment() {
	...
	sleep 1
	aprun -n $PROCESSES hyperPraw -n $EXPERIMENT_NAME"_rHDRF_default_4" -h $HYPERGRAPH_FILE -i 100 -m 1200 -p rHDRF -t $E_SIM_STEPS -x $H_SIM_STEPS -s $SEED -k $MESSAGE_SIZE -H -b $BM_FILE -e $GRAPH_STREAM -P -K 4
	sleep 1
	aprun -n $PROCESSES hyperPraw -n $EXPERIMENT_NAME"_rHDRF_default_8" -h $HYPERGRAPH_FILE -i 100 -m 1200 -p rHDRF -t $E_SIM_STEPS -x $H_SIM_STEPS -s $SEED -k $MESSAGE_SIZE -H -b $BM_FILE -e $GRAPH_STREAM -P -K 8
	sleep 1

...
run_experiment() {
	...
	do
		...
		#aprun -n $PROCESSES hyperPraw -n $EXPERIMENT_NAME"_staggered_overlap_lambda01_parallelVertex_"$MAX_PROCESSES -h $HYPERGRAPH_FILE -i 100 -m 1200 -p parallelVertex -t $E_SIM_STEPS -x $H_SIM_STEPS -s $SEED -k $MESSAGE_SIZE -e $GRAPH_STREAM -P -K $MAX_PROCESSES -g 1 -b $BM_FILE -B -r 100
		#sleep 1
		...
		aprun -n $PROCESSES hyperPraw -n $EXPERIMENT_NAME"_hyperPraw_default_lambda1_"$MAX_PROCESSES -h $HYPERGRAPH_FILE -i 100 -m 1050 -p hyperPrawVertex -t $E_SIM_STEPS -x $H_SIM_STEPS -s $SEED -k $MESSAGE_SIZE -e $GRAPH_STREAM -P -K $MAX_PROCESSES -g 1 -b $BM_FILE -r 1000
		sleep 1
		aprun -n $PROCESSES hyperPraw -n $EXPERIMENT_NAME"_hyperPraw_bandwidth_lambda1_"$MAX_PROCESSES -h $HYPERGRAPH_FILE -i 100 -m 1050 -p hyperPrawVertex -t $E_SIM_STEPS -x $H_SIM_STEPS -s $SEED -k $MESSAGE_SIZE -e $GRAPH_STREAM -P -K $MAX_PROCESSES -g 1 -b $BM_FILE -W -r 1000
		sleep 1
		aprun -n $PROCESSES hyperPraw -n $EXPERIMENT_NAME"_hyperPraw_default_lambda10_"$MAX_PROCESSES -h $HYPERGRAPH_FILE -i 100 -m 1050 -p hyperPrawVertex -t $E_SIM_STEPS -x $H_SIM_STEPS -s $SEED -k $MESSAGE_SIZE -e $GRAPH_STREAM -P -K $MAX_PROCESSES -g 1 -b $BM_FILE -r 10000
		sleep 1
		aprun -n $PROCESSES hyperPraw -n $EXPERIMENT_NAME"_hyperPraw_bandwidth_lambda10_"$MAX_PROCESSES -h $HYPERGRAPH_FILE -i 100 -m 1050 -p hyperPrawVertex -t $E_SIM_STEPS -x $H_SIM_STEPS -s $SEED -k $MESSAGE_SIZE -e $GRAPH_STREAM -P -K $MAX_PROCESSES -g 1 -b $BM_FILE -W -r 10000
		sleep 1
...
do
	SEED=$RANDOM
	#synthetic graphs
	run_experiment "small_dense_uniform.hgr" $SEED 1 8
	run_experiment "small_dense_powerlaw.hgr" $SEED 1 8
	run_experiment "large_sparse_uniform.hgr" $SEED 1 8
	run_experiment "large_sparse_powerlaw.hgr" $SEED 1 8

...
public:
	...
	VertexPartitioning(char* experimentName, char* graph_file, int iterations, float imbalance_tolerance, char* comm_bandwidth_file, bool parallel, bool useBandwidth, bool saveHistory) : Partitioning(graph_file,imbalance_tolerance,false) {
	...
	virtual void perform_partitioning(int num_processes,int process_id) {
        ...
        PRAW::ParallelVertexPartitioning(experiment_name,partitioning, comm_cost_matrix, hedge_ptr.size(), hyperedges.size(), &hyperedges, he_wgt, max_iterations, imbalance_tolerance, save_partitioning_history);
        */

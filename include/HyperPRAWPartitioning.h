...
public:
	...
	virtual void perform_partitioning(int num_processes,int process_id) {
        ...
        if(isParallel) {
            PRAW::ParallelIndependentRestreamingPartitioning(experiment_name,partitioning, comm_cost_matrix, hgraph_name, vtx_wgt, max_iterations, imbalance_tolerance, ta_refinement, reset_partitioning,stopping_condition,save_partitioning_history);
        } else {
            if(process_id == 0) {

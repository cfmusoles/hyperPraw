    ...
    void runSimulation(char* experiment_name, char* graph_file, char* part_method, char* bandwidth_file,  idx_t* partitioning, float partition_timer, int num_vertices, int simulation_iterations, int sim_steps, int hedge_sim_steps_multiplier, int message_size, bool proportional_comm_cost) {
        ...
        for(int iteration=0; iteration < simulation_iterations; iteration++) {
            ...
            if(process_id == 0) {
                ...
                printf("Partition time %.2fs\nVertex replication factor %.3f, %.3f (max imbalance), %.0f (sim comm cost), \nMessages sent %li\n",partition_timer,vertex_replication_factor,max_imbalance,total_sim_comm_cost,total_edge_messages_sent);
                ...
                if(fp == NULL) {
                    ...
                    if(!fileexists) // file does not exist, add header
                        fprintf(fp,"%s,%s,%s,%s,%s,%s\n","Partition time","Sim time","Vertex replication factor","Max imbalance","Sim Comm cost","Sim Messages sent");
                    fprintf(fp,"%.3f,%.3f,%.3f,%.3f,%.0f,%li\n",partition_timer,total_edge_sim_time,vertex_replication_factor,max_imbalance,total_sim_comm_cost,total_edge_messages_sent);
                }
                fclose(fp);
